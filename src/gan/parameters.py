...
flags.DEFINE_string('dataset', DATASET+ "\cifar10",
                    'Dataset to use for training. [image\mnist, protein\wgan\mini_sample, image\cifar10]')
...
flags.DEFINE_integer('batch_size', 2, 'Number of images in input batch. [64]')  # ori 16
...
flags.DEFINE_string('name', 'embedding', 'A name of the model which will be used to construct folder name for weights')
...
if MODEL_TYPE == 'sngan':
    ...
    flags.DEFINE_integer('gf_dim', 16, 'Dimensionality of gf. [64]')
    flags.DEFINE_integer('df_dim', 16, 'Dimensionality of df. [64]')
    flags.DEFINE_string('loss_type', 'hinge_loss',
                        'the loss type can be [hinge_loss, hinge_loss_ra, wasserstein or kl_loss]')
    flags.DEFINE_string('architecture', 'conditional', 'Network architecture. Options: [resnet, conditional, 1d, rnn]')
...
if DATASET == 'image':
    ...
    flags.DEFINE_bool('one_hot', False, 'Whether to use one hot encoding [False]')
    flags.DEFINE_integer('embedding_height', 8, 'The height of embedding used in generator/discriminator')
    ...
    flags.DEFINE_float('variation_level', 1.0, 'Hyper parameter of loss which controls variation. [10]')
    ...
    flags.DEFINE_bool('dynamic_padding', False, 'Whether to use dynamic padding [False]')
